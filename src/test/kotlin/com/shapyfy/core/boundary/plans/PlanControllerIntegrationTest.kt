...
class PlanControllerIntegrationTest : IntegrationTestBase() {
    ...
    fun `should create workout plan successfully`() {
        ...
        createPlan(request, testUserId)
            ...
            .andExpect(jsonPath("$.cycle_days").value(4))
            .andExpect(jsonPath("$.is_active").value(false))
            .andExpect(jsonPath("$.activation_date").doesNotExist())
            .andExpect(jsonPath("$.days.length()").value(4))
            // Verify Day 1 (Push)
            .andExpect(jsonPath("$.days[0].day_index").value(0))
            ...
            .andExpect(jsonPath("$.days[0].exercises[0].exercise_id").value(benchPressId.toString()))
            .andExpect(jsonPath("$.days[0].exercises[0].target_sets").value(4))
            .andExpect(jsonPath("$.days[0].exercises[0].target_reps").value(8))
            .andExpect(jsonPath("$.days[0].exercises[0].target_weight").value(100.0))
            .andExpect(jsonPath("$.days[0].exercises[0].target_formatted").value("4x8 @ 100.0kg"))
            ...
            // Verify Day 4 (Rest)
            .andExpect(jsonPath("$.days[3].day_index").value(3))
    ...
    fun `should create plan with multiple exercises per day`() {
        ...
        createPlan(request, testUserId)
            ...
            .andExpect(jsonPath("$.days[0].exercises[0].exercise_id").value(squatId.toString()))
            .andExpect(jsonPath("$.days[0].exercises[0].order_index").value(0))
            .andExpect(jsonPath("$.days[0].exercises[1].exercise_id").value(legPressId.toString()))
            .andExpect(jsonPath("$.days[0].exercises[1].order_index").value(1))
            .andExpect(jsonPath("$.days[0].exercises[2].exercise_id").value(legCurlId.toString()))
            .andExpect(jsonPath("$.days[0].exercises[2].order_index").value(2))
    }
